...
public class Insert {
    ...
    public static void main(String[] args) {
        ...
        insert(authorValue, tweetValue);
    }
    
    static void insert(String authorValue, String tweetValue) {
        Date dateValue = new Date();
        String idValue = getId();
        Session session = getSession();
        session.currentTransaction().begin();
        // create a new instance of Tweets

        // set values for id, time_stamp, author, and tweet

        // persist the new instance of tweets

        String[] words = tweetValue.split(" ");
        for (String word: words) {
            if (word.startsWith("#")) {
                String hashtagValue = word.substring(1);
                // create a new instance of Hashtags

                // set values for author, hashtag, time_stamp, and tweet_id

                // persist the new instance of hashtags
